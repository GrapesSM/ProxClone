...
int getFileSize(const int index, const int index2) {
...
void playSound(const int index, const int index2, byte SoundFlag, uint8_t volume, uint8_t speed = 10) {
  ...
  if (SoundFlag == true) {
    ...
    if (f.position() < (f.size() - 1)) {
      int numBytes = _min(1024, f.size() - f.position() - 1); // f.size() / 16000 ~ X seconds ~ X * 1000 milliseconds
      ...
      if (i < numBytes) {
        ...
        dacWrite(25, scaled);
        timer1old = micros();
        while (micros() - old < speed); //125usec = 1sec/8000 and assume 5us for overhead like wifi
      } else {
        i = 0;
        byteflag = 0;
      }
...
void playSound(File * filename, uint8_t volume, uint8_t microseconds = 10) 
{
  int i = 0;
  ...
  while (f->position() < (f->size() - 1)) {
    ...
    if (i < numBytes) {
      ...
      dacWrite(25, scaled);
      timer1old = micros();
      while (micros() - old < microseconds); //125usec = 1sec/8000 and assume 5us for overhead like wifi
    } else {
      i = 0;
      byteflag = 0;
    }
